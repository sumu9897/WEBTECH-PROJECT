...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($everythingOkCounter == 0) {
        ...
        $user_data = show_single_data("user_mail", $update_location);
        $user_data["password"] = $newPassword;
        $update_confirmation = update_data("user_mail", $update_location, $user_data);

        if ($update_confirmation) {
            echo "Password Updated Successfully";
            header("Location: ../../view/user/login.php");
        } else {
            header("Location: ../../view/user/Forget_Password/ForgetPassword.php");
        }
        ...
        header('Location:../../view/user/changepassword.php');
    }
}
